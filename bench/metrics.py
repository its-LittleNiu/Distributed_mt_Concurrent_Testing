...
class MetricsCollector:
    ...
    def export_worker_payload(self, cpu_bottleneck: bool) -> dict:
        """Serialize this collector for WorkerRunner -> MasterRunner aggregation."""
        with self._lock:
            items = [dataclasses.asdict(m) for m in self._items]
        ...
        return {
            ...
            "peak_inflight": peak,
            "started_at": started,
            "cpu_bottleneck": bool(cpu_bottleneck),
        }
...
def merge_worker_payloads_for_summary(
    payloads: list[dict],
    concurrency: int,
    request_rate: float,
) -> tuple[dict, bool]:
    """
    Merge per-worker export_worker_payload() dicts into one summary row.
    peak_concurrent_requests uses sum(worker peaks) as a conservative upper bound.
    """
    merged_items: List[RequestMetric] = []
    min_started: Optional[float] = None
    peak_sum = 0
    any_cpu = False
    for p in payloads:
        for d in p.get("items") or []:
            merged_items.append(RequestMetric(**d))
        peak_sum += int(p.get("peak_inflight") or 0)
        s = p.get("started_at")
        if isinstance(s, (int, float)):
            min_started = float(s) if min_started is None else min(min_started, float(s))
        if p.get("cpu_bottleneck"):
            any_cpu = True
    if min_started is None:
        min_started = time.time() - 1.0
    collector = MetricsCollector()
    collector._started_at = min_started
    collector._items = merged_items
    collector._peak_inflight = peak_sum
    summary = collector.summary(concurrency=concurrency, request_rate=request_rate)
    return summary, any_cpu
