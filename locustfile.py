...
def _(environment, **kwargs):
    if isinstance(environment.runner, MasterRunner):
        environment.runner.register_message("trtllm_bench_metrics", _recv_worker_bench_metrics)
    ...
    if isinstance(environment.runner, MasterRunner):
        ...
        if proc_n and len(payloads) < proc_n:
            print(
                ...
                "(missing workers often follow heartbeat/RPC issues; row may be partial)."
            )
        try:
            summary_data, merged_cpu = merge_worker_payloads_for_summary(
                payloads,
            ...
            summary_data.update(
                {
                    ...
                    "model": opts.model,
                    "client_cpu_bottleneck": str(merged_cpu).lower(),
                }
            )
            ...
            if merged_cpu:
                print("[bench] warning: client CPU >= 90% detected on at least one worker")
